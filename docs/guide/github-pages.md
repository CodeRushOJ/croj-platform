...
`Documentation Pages` 工作流，默认站点路径为 `/croj-platform/`。工作流只给构建任务只读仓库权限，部署任务单独获得 `pages: write` 和 OIDC `id-token: write` 权限。
...
Pages 部署使用 `github-pages` environment，建议为生产仓库配置审批与分支保护。回滚时，在目标历史提交上手动运行工作流，或恢复文档提交后合并到 `main`；不要手工编辑构建产物。
