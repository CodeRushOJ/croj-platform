...
class ProductE2EContractTest(unittest.TestCase):
    ...
    def test_browser_journey_drives_real_product_paths_without_network_stubs(self):
        ...
        self.assertIn("const sourceCode =", spec)
        self.assertIn('await editor.press("ControlOrMeta+A");', spec)
        self.assertIn("await editor.pressSequentially(sourceCode);", spec)
        self.assertIn("await expect(editor).toHaveValue(sourceCode);", spec)
        self.assertNotIn('await editor.fill("");', spec)
        self.assertNotIn("await editor.fill(sourceCode);", spec)
        self.assertNotIn("await editor.click();", spec)
        self.assertNotIn("page.keyboard.insertText(", spec)
        self.assertNotIn("page.route(", spec)
        self.assertNotIn("route.fulfill(", spec)
        self.assertNotIn(".evaluate(", spec)
        self.assertNotIn(".$eval(", spec)
        self.assertNotIn(".dispatchEvent(", spec)
    ...
    def test_product_flow_proves_headless_sandbox_discovery_on_both_workers(self):
        ...
        self.assertIn("sandbox_dns_probe_phase", script)
        self.assertIn('kubectl logs "$sandbox_dns_probe_pod"', script)
